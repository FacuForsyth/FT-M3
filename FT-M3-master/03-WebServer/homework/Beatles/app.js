...
http
  .createServer((req, res) => {
    ...
    }  //muestra todo el objeto con los 4 beatles

    if(req.url.substring(0,5) === '/api/' && req.url.length > 5){
      //substring: cuenta caracteres - seria para contar desde /api/... despues va el nombre del beatles 
      let findBeatle = req.url.split('/').pop();  //John%20Lennon
      let foundBeatle = beatles.find((b)=> findBeatle === encodeURI(b.name)) //llevo el nombre del beatle del objeto a codigo(20%) para compararlo
      if(foundBeatle){
        res.writeHead(200, {'Content-type':'application/json'});
  ...
  /*nodemon app.js
  abrir navegador
  localHost:3001/Richard%20Starkey
  */