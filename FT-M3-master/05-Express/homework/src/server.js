...
server.post(PATH, (req, res)=>{
    const {author, title, contents} = req.body;  //tomo el body y activo el server.use(express.json)
...
server.get(PATH, (req,res)=> {
    ...
    if(term){
        ...
        return res.json(term_posts); //por defecto manda un status 200
    }
...
server.get(`${PATH}/:author`, (req, res)=> {
    let {author} = req.params;
    //filter q devuelva un nuevo arreglo con la coincidencia de author
...
server.put(PATH, (req, res)=> {
    ...
    if(id && title && contents){
        //find. devuelve el primer elemento q coincida - solo no necesito para el id pasado por body
        let post = posts.find(p=> p.id === parseInt(id)); //si trabajamos con params o query tiene que ir === parseInt(id) xq es string
        if(post){
            post.title = title,
